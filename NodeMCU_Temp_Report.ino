...
const char* ssid = "VMDF72D64";
const char* password = "meUhumpaYa7z";
...
int db_time = 600 - 1; //desired time -1 to make up for the > sign.
int sensor_time = 3 - 1; //interval time for sensor update
...
void reconnect() {
  while (!mqtt.connected()) {
    if (mqtt.connect("LIVING")) {
      mqtt.publish("LIVING_CLIENT","Connected");
      mqtt.subscribe("LIVING_ROOM_IN");
    } else {
...
void loop() {
  ...
  else
  {
    ...
    if (timepassed - last_time2 > sensor_time)
    {
    ...
    //Send Data to DB.
    if (timepassed - last_time > db_time)
    {
      ...
      mqtt.publish("LIVING_ROOM_OUT",data_arr);
